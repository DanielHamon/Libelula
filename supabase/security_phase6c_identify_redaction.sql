-- ============================================================
-- IAbooks — Fase 6C: ocultación de soluciones de identificar
...
DECLARE
  v_identificar JSONB;
BEGIN
  v_identificar := public.campos_publicos_actividad(
    'identificar',
    '{"opciones":[{"texto":"A","icono":"x","esCorrecta":true}]}'::jsonb
  );

  IF jsonb_path_exists(v_identificar, '$.opciones[*].esCorrecta') THEN
    RAISE EXCEPTION 'fase6c: esCorrecta continúa visible en identificar';
  END IF;

  IF v_identificar #>> '{opciones,0,texto}' <> 'A'
     OR v_identificar #>> '{opciones,0,icono}' <> 'x' THEN
    RAISE EXCEPTION 'fase6c: se eliminó contenido público de identificar';
  END IF;
...
SELECT
  true AS identificar_sin_soluciones,
  true AS opciones_identificar_conservadas,
